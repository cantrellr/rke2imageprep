...
action_image_push() {
    ...
    # Generate RKE2 registries.yaml configuration file
    # This file configures RKE2 to use the private registry for image pulls
...
# Target Registry: ${REGISTRY_URL}
# 
# This file should be copied to /etc/rancher/rke2/registries.yaml on each RKE2 node
# (both server and agent nodes) before starting or restarting RKE2.
#
# For more information, see: https://docs.rke2.io/install/private_registry
...
YAML
        ...
        # Add authentication configuration if credentials were provided
        if [[ "$NO_AUTH" == false ]]; then
            cat >> "$registries_file" << YAML
...
configs:
  "${REGISTRY_URL}":
    auth:
      username: ${USERNAME}
      # NOTE: Replace this with your actual password
      # For production use, ensure this file has restricted permissions (chmod 600)
      password: <YOUR_PASSWORD_HERE>
YAML
        fi
        
        echo "  ✓ Created: $registries_file"
        echo ""
        echo "Next Steps:"
        echo "1. Review and edit $registries_file (add your password if authentication is required)"
        echo "2. Copy $registries_file to /etc/rancher/rke2/registries.yaml on all RKE2 nodes"
        echo "3. Ensure file has restricted permissions: chmod 600 /etc/rancher/rke2/registries.yaml"
        echo "4. Start or restart RKE2 for changes to take effect"
        echo ""
        echo "Example deployment command:"
        echo "  sudo cp $registries_file /etc/rancher/rke2/registries.yaml"
        echo "  sudo chmod 600 /etc/rancher/rke2/registries.yaml"
        echo "  sudo systemctl restart rke2-server  # or rke2-agent"
    fi
